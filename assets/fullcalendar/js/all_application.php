...
function onSubmitStudyplan_viewbtn(){
    ...
    var global1 =     angular.element(document.querySelector('[ng-controller="appCtrl"]')).scope(). temp;

   
    // destroy calender old redring for new rendrind
    ...
    $('#calendar').fullCalendar({
        ...
        eventLimit: true, // allow "more" link when too many events base_url+'calendar/updateEvent', {
    ...
    function modal(data) {
        ...
        $('#details_status').val(data.event ? data.event.details_status : '');

        $('#color').val(data.event ? data.event.color : '#3a87ad');

        $('#comment').val(data.event ? data.event.comment : 'pending');
