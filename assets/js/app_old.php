...
if ($check) {
    ...
    Loading("body", '<?php echo lang("loading_datatable") ?>', "", "show");
    console.log($scope.study3);
        $http.post(base_url + 'study_material/newAssignment', $scope.study3).then(
                function (response) {
                      var otherData = {
                          'class_id': $scope.study3.class,
                          'batch_id': $scope.study3.section,
                          'subject_id': $scope.study3.subject
                      };
                      $scope.getNewMaterials();
                      $scope.study3.files = [];
    ...
    Loading("body", '<?php echo lang("loading_datatable") ?>', "", "show");
        $http.post(base_url + 'study_material/newAssignment', $scope.study4).then(
                function (response) {
                      var otherData = {
                          'class_id': $scope.study4.class,
                          'batch_id': $scope.study4.section,
                          'subject_id': $scope.study4.subject
                      };
                      $scope.getHomework();
                      $scope.study4.files = [];
...
$scope.materialNewDropzoneConfigUpload = {
  ...
  $scope.showAllHomework = function(){
...
app2.controller("syllabusController", function ($scope, $http, $window, $location, $filter) {
    ...
    $scope.request = function(id, status){
        ...
        $scope.confirm_request = function(){
            if($scope.requestText != null){
                Loading("body", '<?php echo lang("loading_datatable") ?>', "", "show");
                $http.post(base_url + 'syllabus/reqForApprovalSyls', {id:$scope.my_new_id,status:$scope.my_new_status,reason:$scope.requestText}, config).then(
                   function (success) {
                   $('.edit_attendance_request_model').modal('hide');
                        showNotification('<?php echo lang("success_app") ?>', success.data.message, "success");
           ...
           interfaceConfigOverwrite: {
               ...
               TOOLBAR_BUTTONS: [
                   'microphone', 'camera', 'desktop', 'fullscreen',
                    'hangup', 'chat', 'recording', 'raisehand', 'download'
                
                   
                    <?php if($sess_role_id == '1' || $sess_role_id == '4'){?>
                        ,'mute-everyone'
                     <?php }?>
           ...
           var options = {
               ...
               interfaceConfigOverwrite: {
                   ...
                   TOOLBAR_BUTTONS: [
                    ...
                    'microphone', 'camera', 'desktop', 'fullscreen',
                    'hangup', 'chat', 'recording', 'raisehand','download','mute-everyone',
   ...
   $scope.joinClassAsStudent = function(class_name, student_name, subject){
       ...
       var options = {
        ...
        api.addEventListener('readyToClose', function(){
         ...
         //console.log("hello ");
           //document.getElementsByClassName('filmstrip').style.visibility = 'hidden';
        //}
   ...
   console.log($scope.requestText);
        ...
        console.log($scope.requestText);
            if($scope.requestText != null){
                ...
                Loading("body", '<?php echo lang("loading_datatable") ?>', "", "show");
                $http.post(base_url + 'study_plan/reqForApprovalSyls', {id:$scope.my_new_id,status:$scope.my_new_status,reason:$scope.requestText}, config).then(
                   function (success) {
                   $('.edit_attendance_request_model').modal('hide');
                        showNotification('<?php echo lang("success_app") ?>', success.data.message, "success");
                        $scope.r_id = $scope.my_new_id;
