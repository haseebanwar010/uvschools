...
function save_image($base64_string) {
    ...
    $source_img = imagecreatefromstring($image_base64);
    ...
    // $angle = '90';
    // $image_base64=imagerotate($image_base64,$angle,0);
...
function sendNotificationViaPusher($msg_key, $recipient, $url, $data) {
    ...
    ////////////////key////////////secret key////////////App Id//////
    $pusher = new Pusher\Pusher(
        'd206b863142875938dd2', 'a102adb851f924741749', '1241611', $options
    );
    ...
    // $pusher->trigger('mychanal-23638', 'my-event', $msg);
...
function check_user_permissions() {
    ...
    if (isset($array)) {
        ...
        // messages add
        $arr2 = array('permission' => 'messages-show', 'val' => 'false');

        $arr3 = array('permission' => 'applications-all', 'val' => 'false');
        ...
        foreach ($array as $key => $value) {
            if (($value->permission == 'messages-employee' && $value->val == 'true') || ($value->permission == 'messages-parent' && $value->val == 'true') || ($value->permission == 'messages-student' && $value->val == 'true'))
            {
                $arr2 = array('permission' => 'messages-show', 'val' => 'true');
                 break;
            }
        }
        foreach ($array as $key => $value) {
            ...
            if ($value->permission == 'applications-student' && $value->val == 'true'){
                $arr3 = array('permission' => 'applications-all', 'val' => 'true');
        ...
        $arr4 = array('permission' => 'dashboard-index', 'val' => 'true');
        //array_push($array,$arr);
        $array[] = (object) $arr4;
       // print_r($array);die();
        $userdata = $ci->session->userdata("userdata");
        $userdata["persissions"] = json_encode($array);
...
//     $tags["total_deductions"] = $data[count($data)-1]->deduction_amount-$data[0]->other_deductions;;
...
//     $tags["other_deductions"] = $data[0]->other_deductions;
//     $tags["other_deduction_remarks"] = $data[0]->other_deduction_remarks;
...
function render_payroll_tags($data, $html, $lang_id)
{
    ...
    $tags["employee_name"] = $data[count($data)-1]->employee_name;
    $tags["employee_category"] = $data[count($data)-1]->category_name;
    $tags["basic_salary"] = $data[count($data)-1]->basic_salary;
    // $tags["allowances"] = $data[count($data)-1]->total_amount - ($data[count($data)-1]->salary_amount + $data[count($data)-1]->deduction_amount);
    $tags["allowances"] = $data[count($data)-1]->additional_payment;
    // $tags["total_paid"] = $total_paid = $data[count($data)-1]->salary_amount + $data[count($data)-1]->total_amount - ($data[count($data)-1]->salary_amount + $data[count($data)-1]->deduction_amount);
    $tags["total_paid"] = $total_paid = $data[count($data)-1]->basic_salary + $data[count($data)-1]->additional_payment;
    $tags["deductions"] = $data[count($data)-1]->deduction_amount;
    $tags["total_deductions"] = $data[count($data)-1]->deduction_amount-$data[0]->other_deductions;;
    $tags["payment_date"] = to_html_date(explode(" ",$data[count($data)-1]->created_at)[0]);
    $tags["employee_department"] = $data[count($data)-1]->department_name;
    ...
    foreach($data as $key => $d)
    {
        ...
        if($lang_id==1)
        {
            $testing_var .= '
            <tr>
                <td style="text-align: center; width: 33.3%;" class="border-right">'.$counter.'</td>
                <td style="text-align: center; width: 33.3%;" class="border-right">'.$d->amount_paid.'</td>
                <td style="text-align: center; width: 33.3%;">'.$d->date.'</td>
            </tr>';
        }
        else
        {
            $testing_var .= '
            <tr>
                <td style="text-align: center; width: 33.3%;" class="border-left">'.$counter.'</td>
                <td style="text-align: center; width: 33.3%;" class="border-left">'.$d->amount_paid.'</td>
                <td style="text-align: center; width: 33.3%;">'.$d->date.'</td>
            </tr>';
        }
        ...
        $total_netAmount=$total_netAmount+$d->amount_paid;
    }
    
    $tags["net_payment"]=$total_netAmount;
    
    $tags["installments_data"]=$testing_var;
...
function render_fee_tags($data, $html, $lang_id) {
    ...
    if($data["data"]->feetype_amount == 0){
        ...
        $tags["discount_percentage"] = 100-($data["data"]->discount_amount/$data["data"]->feetype_amount)*100;
...
function render_resultcard_tags_api($data, $html) {
    ...
    $tags["school_logo"] = "<img src='$logo' id='school-logo' style='width:150px;'/>";
    // $tags["logged_in_user"] = $ci->session->userdata("userdata")["name"];
...
function render_resultcard_tags($data, $html) {
    ...
    if(isset($data["data"]["details"][0]->activities))
    {
        if(count($data["data"]["details"][0]->activities) == 0)
        {
            $data['template']->html = str_replace('<td style="text-align: center; width: 16.3757%;"><strong>Details</strong></td>',"",$data['template']->html);
            $data['template']->html = str_replace('<td style="text-align: center;"><strong>تفاصيل</strong></td>',"",$data['template']->html);
            $data['template']->html = str_replace('<table id="result-marks-table" style="border: 1px solid black; margin-top: 15px; font-size:10px;" border="1px solid" width="100%">','<table id="result-marks-table" style="border: 1px solid black; margin-top: 15px; font-size:14px;" border="1px solid" width="100%">',$data['template']->html);
        }
    }
    
    // else
    // {
    //     $data['template']->html = str_replace('<table id="result-marks-table" style="border: 1px solid black; margin-top: 15px; font-size:10px;" border="1px solid" width="100%">','<table id="result-marks-table" style="border: 1px solid black; margin-top: 15px; font-size:12px;" border="1px solid" width="100%">',$data['template']->html);
    // }
    ...
    $tags["school_country"] = $ci->admin_model->dbSelect("country", "school", " id='$school_id' ")[0]->country; //froms ession
    $tags["school_city"] = $ci->admin_model->dbSelect("city", "school", " id='$school_id' ")[0]->city; //froms ession
    $tags["school_logo"] = "<div class='top_header_logo'><img src='$logo' id='school-logo' style='width:150px;'/></div>";
    ...
    foreach($data["data"]["details"] as $d) {
        if($d->total_obtained_marks != 'null' || !empty($d->total_obtained_marks)) { 
            ...
            if($d->type == 'number'){
                if ((strpos($html->html, '<strong>Subjects</strong>') !== false) || strpos($html->html, '<strong>اسم المادة</strong>') !== false) { 
                    $result_details .= '<tr><td style="text-align: center;">'.$d->subjnect_name.'</td>';
                } else {
                    $result_details .= '<tr>';
                }
                
                if ((strpos($html->html, '<strong>Total Marks</strong>') !== false) || (strpos($html->html, '<strong>الدرجة الكاملة</strong>') !== false)) {
                    $result_details .= '<td style="text-align: center;">'.$d->total_marks.'</td>';
                }

                // sheraz update 09-06-2021
                if (isset($d->activities) && count($d->activities) > 0) {
                    if (strpos($html->html, '<strong>Details</strong>') !== false || strpos($html->html, '<strong>تفاصيل</strong>') !== false) {
                        $result_details .= '<td>';
                        $result_details .= '<span style="display:block; float:left; padding-left:5px;">Exam Marks</span><span style="display:block; float:right; padding-right:5px;">'.$d->obtained_marks.'/'.$d->total_written_marks.'</span><br>';
                        foreach ($d->activities as $key => $ac) {
                            if(isset($ac->$key->obtained_marks) && $ac->$key->obtained_marks != null){
                                $result_details .= '<span style="display:block; float:left; padding-left:5px;">'.$ac->activity_name.'</span><span style="display:block; float:right; padding-right:5px;">'.$ac->$key->obtained_marks.'/'.$ac->marks.'</span></br>';
               ...
               // $result_details .= '</td>';
               // sheraz update 09-06-2021
                if (strpos($html->html, '<strong>Obtained Marks</strong>') !== false || strpos($html->html, '<strong>Total Obtain Marks</strong>') !== false || strpos($html->html, '<strong>درجة الطالب</strong>') !== false) {
                    $result_details .= '<td style="text-align: center;">'.$d->total_obtained_marks.'</td>';
                }
                if (strpos($html->html, '<strong>Grade</strong>') !== false || strpos($html->html, '<strong>التقدير</strong>') !== false) {
                    $result_details .= '<td style="text-align: center;">'.$sub_grade.'</td>';   
                }
                if (strpos($html->html, '<strong>Status</strong>') !== false || strpos($html->html, '<strong>الحالة</strong>') !== false) {
                    $result_details .= '<td style="text-align: center;">'.lang(strtolower($d->status)).'</td>';      
                }
                if (strpos($html->html, '<strong>Remarks</strong>') !== false || strpos($html->html, '<strong>ملاحظات</strong>') !== false) {
                    $result_details .= '<td style="text-align: center;">'.$d->remarks.'</td>';      
                }
                ...
                if ((strpos($html->html, '<strong>Total Marks</strong>') !== false) || (strpos($html->html, '<strong>الدرجة الكاملة</strong>') !== false)) {
                    $result_details .= '<td style="text-align: center;">-</td>';
                }
                if (strpos($html->html, '<strong>Obtained Marks</strong>') !== false || strpos($html->html, '<strong>Total Obtain Marks</strong>') !== false || strpos($html->html, '<strong>درجة الطالب</strong>') !== false ) {
                    $result_details .= '<td style="text-align: center;">-</td>';
                }
                if (strpos($html->html, '<strong>Grade</strong>') !== false || strpos($html->html, '<strong>التقدير</strong>') !== false) {
                    $result_details .= '<td style="text-align: center;">'.$d->grade.'</td>';
                }
                if (strpos($html->html, '<strong>Status</strong>') !== false || strpos($html->html, '<strong>الحالة</strong>') !== false) {
                    $result_details .= '<td style="text-align: center;">-</td>';
                }
...
<tr class="hidden_row">
<td style="width: 90%; font-size: 30px;" colspan="7">{result_details}</td>
</tr>
<tr>
...
function system_result_card_template_arabic(){
    ...
    <tr class="hidden_row"><td colspan="7">{result_details}</td></tr>
    <tr>
...
function system_payroll_template_english(){
    ...
    <table width="100%" style="border-bottom: 1px solid;">
    <tr style="text-align: center;">
    <td style="border-right: 1px solid; width: 20%;"><strong>Description</strong></td>
    <td style="border-right: 1px solid; width: 20%;"><strong>Earnings</strong></td>
    <td style="width: 20%; border-right: 1px solid;"><strong>Deductions</strong></td>
    <td style="width: 20%;  border-right: 1px solid;"><strong>Other Deductions</strong></td>
    <td style="width: 20%;"><strong>Other Deduction Remarks</strong></td>
    </tr>
    </table>
    <table width="100%" style="border-bottom: 1px solid;">
    <tr>
    <td style="width:20%; padding-left: 8px;" class="border-right">
    <span>Basic Salary</span>
    <br/>
    <span>Allowances</span>
    </td>
    <td style="text-align: center; width: 20%;" class="border-right">
    ...
    <tr>
    <td style="width: 20%; padding-left: 8px;" class="border-right"><strong>Total</strong></td>
    <td style="text-align: center; width: 20%;" class="border-right"><strong>{total_paid}</strong></td>
    <td style="text-align: center; width: 60%;"><strong>{total_deductions}</strong></td>
    </tr>
    </table>
...
function system_payroll_template_arabic(){
    ...
    <table width="100%" style="border-bottom: 1px solid;">
    <tr style="text-align: center;">
    <td style="border-left: 1px solid; width: 20%;"><strong>وصف</strong></td>
    <td style="border-left: 1px solid; width: 20%;"><strong>أرباح</strong></td>
    <td style="border-left: 1px solid; width: 20%;"><strong>الخصومات</strong></td>
    <td style="width: 20%;  border-left: 1px solid;"><strong>خصومات أخرى</strong></td>
    <td style="width: 20%;"><strong>ملاحظات عن الخصم الأخرى</strong></td>
    </tr>
    </table>
    <table width="100%" style="border-bottom: 1px solid;">
    <tr>
    <td style="width:20%; padding-right: 8px;" class="border-left">
    <span>راتب اساسي</span>
    <br/>
    <span>البدلات</span>
    </td>
    <td style="text-align: center; width: 20%;" class="border-left">
    <span>{basic_salary}</span>
    <br/>
    <span>{allowances}</span>
    </td>
