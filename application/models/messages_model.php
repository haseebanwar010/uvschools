...
class Messages_model extends CI_Model {
    ...
    public function getConversations($user_id) {
        $query = $this->db->select('sh_conversations.*,sh_participants.is_read,sh_users.role_id, sh_users.name')
                ->select('date_format(max(sh_messages.created_at),"%b %d") as last_time', false)
                ->select('count(CASE when attachments != null or attachments!="" then 1 end) as att')
    ...
    public function getRecipients($q, $sh_id, $user_id, $role_id) {
        ...
        if ($role_id == 'all') {
            ...
            $query1 = $this->db->
                    select('sh_users.id, sh_users.role_id, sh_users.name as user_name, sh_users.avatar,  sh_role_categories.category,sh_departments.name as department')
                    ->like('sh_users.name', $q)
                    ->where('sh_users.deleted_at', 0)
                    ->where('sh_users.school_id', $sh_id)
                    ->where('sh_users.role_id <>', 3)
                    ...
                    ->get();
            $query2 = $this->db->
                    select('sh_students_'.$school_id.'.id, sh_students_'.$school_id.'.role_id, sh_students_'.$school_id.'.name as user_name, sh_students_'.$school_id.'.avatar, sh_classes.name as class_name,sh_batches.name as batch_name, sh_role_categories.category,sh_departments.name as department')
                    ->like('sh_students_'.$school_id.'.name', $q)
                    ->where('sh_students_'.$school_id.'.deleted_at', 0)
                    ->where('sh_students_'.$school_id.'.school_id', $sh_id)
                    ->where_not_in('sh_students_'.$school_id.'.id', array($user_id))
                    ->from('sh_students_'.$school_id)
                    ->join('sh_classes', 'class_id = sh_classes.id', 'left')
                    ->join('sh_batches', 'batch_id = sh_batches.id', 'left')
                    ->join('sh_role_categories', 'role_category_id = sh_role_categories.id', 'left')
                    ->join('sh_departments', 'sh_students_'.$school_id.'.department_id = sh_departments.id', 'left')
                    ->get();
    ...
    public function getFirstFiveConversations($user_id) {
        $query = $this->db->select('sh_conversations.*,sh_participants.is_read,sh_users.role_id, sh_users.name, sh_users.avatar')
                ->select('date_format(max(sh_messages.created_at),"%b %d") as last_time', false)
                ->select('count(CASE when attachments != null or attachments!="" then 1 end) as att')
                ->from('sh_conversations')
                ->join('sh_participants', 'sh_conversations.id = sh_participants.conversation_id', 'left')
