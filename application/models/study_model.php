...
class Study_model extends CI_Model {
    ...
    public function newAssignment($title, $type, $class_id, $batches, $subject_id, $storage_type, $files, $allfileurls, $allthumbnail_links, $all_fnames, $all_fileIds, $school_id, $details, $material_details,$uploaded_by,$uploaded_at, $subject_code, $marks, $due_date, $student_ids) {
        $data = array(
            'title' => $title,
            'content_type' => $type,
            'class_id' => $class_id,
    ...
    public function getParentsMultiple($student_ids){
        ...
        $merged_user = array_map (function($value){
            return $value['guardian_id'];
        } , $temp);
        
        if ($this->session->userdata('userdata')['role_id'] == 4) {
            $admin_user = $this->db->select('id')->from('sh_users')->where('school_id',$school_id)->where('role_id', 1)->get()->result_array();
            $arr3 = array_map (function($value){
                return $value['id'];
            } , $admin_user);
            
            $merged_user = array_merge($merged_user,$arr3);
        }
    ...
    public function getStudentsAndParents($class_id,$batch_id,$school_id){
        ...
        // update send notification to admin
        
            $merged_user = array_merge($arr,$arr2);
            ...
            if(strlen($batch_id) >= 5)
            {
                ...
                else
                {
                    if ($subject_id != 'all' && $subject_id != '') {
                        $ahoooo=$this->db->select('*')->from('sh_subjects')->where('id',$subject_id)->get()->result();
                        foreach($ahoooo as $rec_aho)
                        {
                            $storage_val[]=$rec_aho->code;
                            $storage_val_ids[]=$rec_aho->id;
                    ...
                    {
                        $ahoooo=$this->db->select('*')->from('sh_subjects')->where('school_id',$school_id)->where('class_id',$class_id)->where("batch_id IN (".$batch_id.")",NULL, false)->where('deleted_at',NULL)->where('academic_year_id',$this->session->userdata("userdata")["academic_year"])->get()->result();
                        foreach($ahoooo as $rec_aho)
                        {
                            $storage_val[]=$rec_aho->code;
                            $storage_val_ids[]=$rec_aho->id;
