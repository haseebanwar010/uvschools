...
//        }
    ...
    function parent_class_work(){
        $this->load->view('study_material/parent_class_work');
    }
    ...
    function getSubjectsForStudent(){
        ...
        if(count($student) > 0){
            ...
            {
                if($value["uploaded_at"] > date("Y-m-d") && ($value["content_type"] == "Assignment" || $value["content_type"] == "Homework"  || $value["content_type"] == "Classwork"  || $value["content_type"] == "Study Material"))
                {
                    unset($materials[$key]);
                }
                else
        ...
        {
            if($data['materials'][$i]['files']!='')
            {
                $data['materials'][$i]['files'] = explode(",", $data['materials'][$i]['files']);
            }
            else
            {
                $data['materials'][$i]['files']='';
            }
            
            if($data['materials'][$i]['filesurl']!='')
            {
                $data['materials'][$i]['filesurl'] = explode(",", $data['materials'][$i]['filesurl']);
            }
            else
            {
                $data['materials'][$i]['filesurl']='';
            }
            
            if($data['materials'][$i]['file_names']!='')
            {
                $data['materials'][$i]['file_names'] = explode(",", $data['materials'][$i]['file_names']);
            }
            else
            {
                $data['materials'][$i]['file_names']='';
            }
            
            if($data['materials'][$i]['thumbnail_links']!='')
            {
                $data['materials'][$i]['thumbnail_links'] = explode(",", $data['materials'][$i]['thumbnail_links']);
            }
            else
            {
                $data['materials'][$i]['thumbnail_links']='';
            }
        }
    ...
    function filter_parent() {
        ...
        $materials = $this->study_model->filter_parentmaterial($school_id, $class_id, $batch_id, $subject_id, $type);
        ...
        for ($i = 0; $i < count($response['materials']); $i++)
        {
            ...
            if($response['materials'][$i]['files']!='')
            {
                $response['materials'][$i]['files'] = explode(",", $response['materials'][$i]['files']);
            }
            else
            {
                $response['materials'][$i]['files']='';
            }
            ...
            {
                $response['materials'][$i]['filesurl']='';
            }
            
            if($response['materials'][$i]['file_names']!='')
            {
                $response['materials'][$i]['file_names'] = explode(",", $response['materials'][$i]['file_names']);
            }
            else
            {
                $response['materials'][$i]['file_names']='';
            }
            
            if($response['materials'][$i]['thumbnail_links']!='')
            {
                $response['materials'][$i]['thumbnail_links'] = explode(",", $response['materials'][$i]['thumbnail_links']);
            }
            else
            {
                $response['materials'][$i]['thumbnail_links']='';
            }
    ...
    function filter_student_all_assignment() {
        ...
        foreach ($response as $res) {
            ...
            $re = $this->db->query("SELECT sm.*,u.avatar as student_avatar, u.name as student_name FROM sh_submit_material sm LEFT JOIN sh_users u ON sm.student_id=u.id WHERE sm.material_id='$res->id' AND sm.student_id='$student_id'")->result();

            $count = count($re);
            if ($count > 0 ) {
                $res->submit_material_id = $re[0]->id;
                $res->student_name = $re[0]->student_name;
                $res->student_avatar = $re[0]->student_avatar;
                $res->remarks = $re[0]->remarks;
                $res->submitted_details = $re[0]->submitted_details;
                $res->submitted_files = explode(',', $re[0]->submitted_files);
                $res->submitted_filesurl=explode(',',$re[0]->filesurl);
                $res->submitted_thumbnail_links=explode(',',$re[0]->thumbnail_links);
                $res->submitted_file_names=explode(',',$re[0]->file_names);
    ...
    function getChilds(){
        ...
        {
            $student = $this->db->select('s.id,s.class_id, s.batch_id, s.subject_group_id')->from('sh_students_'.$school_id.' s')->join('sh_users u', 'u.id = s.id')->where('s.id', $child->student_id)->get()->row();
            if($student)
            {
                    // $b_n = $this->db->select('name')->from('sh_batches')->where('school_id' , $school_id)->where('class_id' , $student->class_id)->where('id' , $student->batch_id)->where('deleted_at is NULL' , NULL)->where('academic_year_id',$this->session->userdata("userdata")["academic_year"])->get()->row();
                    // echo '<pre>';
                    ...
                    if($student->subject_group_id != "" && $student->subject_group_id != null && $student->subject_group_id != 0)
                    {
                        ...
                        if(!empty($subject_ids))
                        {
                            ...
                            $response = $this->db->query("SELECT a.*,c.name as class_name,sb.name as subject_name,u.name as teacher_name,u.id as teacher_id,u.avatar FROM sh_assignments a LEFT JOIN sh_classes c ON a.class_id=c.id LEFT JOIN sh_subjects sb ON a.subject_id=sb.id LEFT JOIN sh_users as u ON a.uploaded_by=u.id WHERE FIND_IN_SET('$student->id',a.student_ids) AND a.school_id='$school_id' AND a.deleted_status='0' AND a.published_date <= '$current_date' ORDER BY a.created_at desc")->result();
                            

                            foreach ($response as $res) {

                                $re = $this->db->query("SELECT sm.*,u.avatar as student_avatar, u.name as student_name FROM sh_submit_material sm LEFT JOIN sh_users u ON sm.student_id=u.id WHERE sm.material_id='$res->id' AND sm.student_id='$student->id'")->result();
                            ...
                            foreach ($response as $key => $mt) {
                                array_push($materials, $mt);
                            }
    ...
    function filter_class_work_parent(){
        $postdata = file_get_contents("php://input");
        $request = json_decode($postdata);
        $school_id = $this->session->userdata("userdata")["sh_id"];
        $current_date = date('Y-m-d');
        ...
        $response = $this->db->query("SELECT a.*,c.name as class_name,sb.name as subject_name,u.name as teacher_name,u.id as teacher_id,u.avatar FROM sh_assignments a LEFT JOIN sh_classes c ON a.class_id=c.id LEFT JOIN sh_subjects sb ON a.subject_id=sb.id LEFT JOIN sh_users as u ON a.uploaded_by=u.id WHERE FIND_IN_SET('$request->student_id',a.student_ids) AND a.school_id='$school_id' AND a.content_type='$request->type' AND subject_id='$request->subject_id' AND a.deleted_status='0' AND a.published_date <= '$current_date' ORDER BY a.created_at desc")->result();


        foreach ($response as $res) {

            $re = $this->db->query("SELECT sm.*,u.avatar as student_avatar, u.name as student_name FROM sh_submit_material sm LEFT JOIN sh_users u ON sm.student_id=u.id WHERE sm.material_id='$res->id' AND sm.student_id='$request->student_id'")->result();
    ...
    function filter_student_all_homework(){
        ...
        foreach ($response as $res) {
            ...
            if ($count > 0 ) {
                ...
                $res->status = "Due";
                $res->obtained_marks = "Submit Your Homework First";
            }
        }
...
public function do_upload($fileName, $dirName) {
    ...
    public function newAssignment(){
        ...
        // push notifiction issue
        $parents = $this->study_model->getParentsMultiple($student_ids);
        $student_id = explode(',', $student_ids);
        $response['part'] = array_merge($parents,$student_id);
    ...
    public function newMaterial_general() {
        ...
        $random_id = "__yasir".generateRandomString(10);
        if(strpos($details, 'target="_blank"'))
        {
            $details .= '<script>
                $(document).ready(function(){
                    $(".'.$random_id.'").magnificPopup({
            ...
            $details = str_replace('<a href', '<a target="_blank" href', $details); 
    ...
    public function book_shop_for_student(){

        
        $this->load->view('study_material/bookshop_for_students');
        
    }

    public function book_shop_for_parent(){

        
        $this->load->view('study_material/bookshop_for_parents');
    ...
    public function get_bookshop(){
        ...
        $xcrud->relation('class_id','sh_classes','id','name','deleted_at IS NULL and school_id = ' . $this->session->userdata("userdata")["sh_id"] .' and academic_year_id = ' . $this->session->userdata("userdata")["academic_year"]);
    ...
    //bookshop module for students and parents added by sheraz #BSI
    public function get_bookshop_for_student(){
        $user_id = $this->session->userdata('userdata')["user_id"];
        $academic_year_id = $this->session->userdata('userdata')["academic_year"];
        $school_id = $this->session->userdata('userdata')["sh_id"];
        $info = $this->db->query("SELECT class_id FROM sh_student_class_relation WHERE student_id='$user_id' AND academic_year_id='$academic_year_id' AND school_id='$school_id'")->result();
        if (count($info) > 0) {
              $class = $info[0]->class_id;
        } else {
            $class = "";
        }  
        $xcrud = xcrud_get_instance();
    ...
    public function get_books_of_student(){
        $postdata = file_get_contents("php://input");
        $request = json_decode($postdata);
        $academic_year_id = $this->session->userdata('userdata')["academic_year"];
        $school_id = $this->session->userdata('userdata')["sh_id"];
        $student_id = $request->student_id;
        $info = $this->db->query("SELECT class_id FROM sh_student_class_relation WHERE student_id='$student_id' AND academic_year_id='$academic_year_id' AND school_id='$school_id'")->result();
        if (count($info) > 0) {
              $class = $info[0]->class_id;
        } else {
            $class = "";
    ...
    public function get_all_books_for_students(){
        $user_id = $this->session->userdata("userdata")["user_id"];
        $school_id = $this->session->userdata("userdata")["sh_id"];
        $academic_year_id = $this->session->userdata("userdata")["academic_year"];
        $class_ids = array();
        $childrens = $this->db->select('student_id')->from('sh_student_guardians sg')->join('sh_students_'.$school_id.' s', 'sg.student_id = s.id')->where('guardian_id', $user_id)->get()->result();
        foreach ($childrens as $key => $child) {
            $info = $this->db->query("SELECT class_id FROM sh_student_class_relation WHERE student_id='$child->student_id' AND academic_year_id='$academic_year_id' AND school_id='$school_id'")->result();
            if (count($info) > 0) {
              array_push($class_ids, $info[0]->class_id);
            }
        }
        $class = array_unique($class_ids);
        $xcrud = xcrud_get_instance();
        $xcrud->table('sh_book_shop');
        $xcrud->where("school_id", $this->session->userdata("userdata")["sh_id"])->where('deleted_at IS NULL');
        $xcrud->where('class_id',$class);
    ...
    public function student_class_activities(){
        ...
        if($role_id == 1 || $role_id == 4){
            ...
            redirect(site_url('study_material/parent_download'));
            redirect(site_url('study_material/parent_class_work'));
        }else if($role_id == 3){
            //redirect(site_url('study_material/student_download'));
            $this->load->view('study_material/student_class_activities');
        }
    ...
    public function submitAssignment() {
        ...
        $sort_files_1=array_unique($request->files,SORT_REGULAR);
        $files_1=array_values($sort_files_1);
        $content_type="Submitted Assignments";
        
        if($request->details->content_type=='Assignment')
        {
            $content_type="Submitted Assignments";
        }
        else
        {
            $content_type="Submitted Homeworks";
        }
    ...
    public function currentDateSubmittedAss(){
                ...
                foreach ($student_ids as $key2 => $std) {
                    ...
                    if($std){
                       $query = "SELECT "
                        ...
                        . "sub_mat.id as submit_id,sub_mat.student_id, sub_mat.date, sub_mat.submitted_files, sub_mat.filesurl as submit_filesurl, sub_mat.thumbnail_links as submit_thumbnail_links, sub_mat.file_names as submit_file_names, sub_mat.submitted_details, sub_mat.obtained_marks, sub_mat.remarks, sub_mat.viewed, date_format(sub_mat.created_at,'%Y-%m-%d') as submitted_date"
        ...
        foreach ($data1 as $key => $data) {
            ...
            for ($i = 0; $i < count($data[0]['assignments']); $i++) {
                ...
                $data[0]['assignments'][$i]->filesurl=explode(',',$data[0]['assignments'][$i]->filesurl);
                $data[0]['assignments'][$i]->thumbnail_links=explode(',',$data[0]['assignments'][$i]->thumbnail_links);
                $data[0]['assignments'][$i]->file_names=explode(',',$data[0]['assignments'][$i]->file_names);
                
                $data[0]['assignments'][$i]->submitted_files = explode(",", $data[0]['assignments'][$i]->submitted_files);
                $data[0]['assignments'][$i]->submitted_filesurl=explode(',',$data[0]['assignments'][$i]->submit_filesurl);
                $data[0]['assignments'][$i]->submitted_thumbnail_links=explode(',',$data[0]['assignments'][$i]->submit_thumbnail_links);
                $data[0]['assignments'][$i]->submitted_file_names=explode(',',$data[0]['assignments'][$i]->submit_file_names);
            }
    ...
    public function currentDateSubmittedHom(){
        ...
        foreach ($data1 as $key => $data) {
            ...
            for ($i = 0; $i < count($data[0]['Homeworks']); $i++) {
                ...
                $data[0]['Homeworks'][$i]->filesurl=explode(',',$data[0]['Homeworks'][$i]->filesurl);
                $data[0]['Homeworks'][$i]->thumbnail_links=explode(',',$data[0]['Homeworks'][$i]->thumbnail_links);
                $data[0]['Homeworks'][$i]->file_names=explode(',',$data[0]['Homeworks'][$i]->file_names);
                
                $data[0]['Homeworks'][$i]->submitted_files = explode(",", $data[0]['Homeworks'][$i]->submitted_files);
                $data[0]['Homeworks'][$i]->submitted_filesurl=explode(',',$data[0]['Homeworks'][$i]->submit_filesurl);
                $data[0]['Homeworks'][$i]->submitted_thumbnail_links=explode(',',$data[0]['Homeworks'][$i]->submit_thumbnail_links);
                $data[0]['Homeworks'][$i]->submitted_file_names=explode(',',$data[0]['Homeworks'][$i]->submit_file_names);
            }
    ...
    public function set_viewed_assignment_homework(){
        ...
        $response = $this->db->query("SELECT c.*, u.name, u.avatar FROM sh_comments as c LEFT JOIN sh_users u ON c.sender_id=u.id  WHERE c.submit_material_id='$request->assignment_id' ")->result();
        ...
        $this->db->update('sh_submit_material', array('viewed' => true));
